...
abstract class SemanticdbSuite extends FunSuite with DiffAssertions { self =>
  ...
  private lazy val databaseOps: SemanticdbOps { val global: self.g.type } = new SemanticdbOps {
    ...
    config = config.copy(failures = FailureMode.Error, text = BinaryMode.On, synthetics = BinaryMode.On)
  ...
  def synthetics(code: String)(fn: (s.TextDocument, m.Tree) => Unit): Unit = {
    import m._
    val db = computeDatabaseFromSnippet(code)
    val tree = db.text.parse[m.Source].get

    test(code)(fn(db, tree))
  }
